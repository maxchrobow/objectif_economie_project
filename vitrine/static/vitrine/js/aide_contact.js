  ...
  if (etape == 0)
  {
	    ...
	    for (i=0; i < 2; i++)//recherche quel bouton sujet est actif dans les 2 premiers(l'animation change pour les 3 derniers)
	{
		if(x[i].classList.contains("active"))
		{
			...
			y[i].classList = "step2 display-block-bouton display-block in_d_g";//display block la section associé au bouton sujet actif
			setTimeout(function(){document.getElementById("step1").classList = "display-none";},200);//display none les boutons sujet step1
			return;
		}
	}
  ...
  else if (etape == 1)
  {
  	if(n == -1)//retour en arrière étape 1
  	{
  		...
  		for (i=0; i < 2; i++)//recherche la section display block dans l'étape 2 pour les 2 premiere section
		{
			...
			{
				y[i].classList = "step2 display-block-bouton display-block";
				var a = y[i]
				a.classList = "out_g_d";//lance l'animation sur les boutons step2
				//display block les boutons sujet step1
				setTimeout(function(a){document.getElementById("step1").classList = "step1 display-block in_g_d";
					a.classList = "step2 display-none";},300,a);//display none les boutons step2
				return;
			}
		}
